...
const CoursesPage = () => {
  ...
  return (
    <section className="w-full bg-linear-to-br from-emerald-50 via-white to-cyan-50 p-4 sm:p-6 lg:p-10 dark:from-emerald-950 dark:via-gray-900 dark:to-cyan-950">
      <div className="mx-auto w-full max-w-6xl">
        <Reveal>
          <div className="relative overflow-hidden bg-linear-to-r from-emerald-500 via-emerald-400 to-cyan-400 p-6 text-white sm:p-8 lg:p-12">
            <div className="absolute -right-8 -top-8 h-40 w-40 bg-white/20 blur-2xl" />
            <div className="absolute -bottom-12 left-16 h-44 w-44 bg-emerald-200/30 blur-2xl" />
            ...
            <div className="relative mt-6 flex flex-wrap gap-3">
              <button className="rounded-full bg-white px-5 py-2.5 text-sm font-semibold text-emerald-700 transition hover:bg-emerald-50 sm:text-base dark:bg-gray-900 dark:text-emerald-300 dark:hover:bg-gray-800">
                Enroll Now
              </button>
              <button className="rounded-full px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-white/10 sm:text-base">
                Download Syllabus
              </button>
            </div>
          </div>
        ...
        <div className="mt-8 grid gap-5 lg:grid-cols-3">
          {popularCourses.map((course, i) => (
            <Reveal key={course.title} direction="up" delay={i * 100}>
              <article
                className="rounded-2xl bg-white p-5 transition hover:-translate-y-1 dark:bg-gray-800"
              >
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-emerald-700 dark:text-emerald-400">Popular</p>
                <h3 className="mt-2 text-lg font-semibold text-neutral-900 dark:text-white">{course.title}</h3>
                <p className="mt-3 text-sm text-neutral-600 dark:text-neutral-300">{course.description}</p>
                <div className="mt-4 flex flex-wrap gap-2 text-xs font-medium text-emerald-800 dark:text-emerald-200">
                  <span className="rounded-full bg-emerald-100 px-3 py-1 dark:bg-emerald-900/40">{course.duration}</span>
                  <span className="rounded-full bg-cyan-50 px-3 py-1 dark:bg-cyan-900/40">{course.mode}</span>
                </div>
              </article>
            </Reveal>
          ))}
        ...
        <div className="mt-8 grid gap-5 lg:grid-cols-2">
          {Object.entries(allCourses).map(([exam, courses], i) => (
            <Reveal key={exam} direction={i === 0 ? 'left' : 'right'}>
              <section className="rounded-2xl bg-white p-6 dark:bg-gray-800">
                <h2 className="text-2xl font-semibold text-neutral-900 dark:text-white">{exam} Preparation Courses</h2>
                <ul className="mt-4 space-y-3">
                  {courses.map((course) => (
                    <li
                      key={course}
                      className="rounded-xl bg-emerald-50/60 px-4 py-3 text-sm text-neutral-700 sm:text-base dark:bg-emerald-900/20 dark:text-neutral-300"
                    >
                      {course}
                    </li>
                  ))}
                </ul>
                <button className="mt-5 bg-emerald-600 px-5 py-2 text-sm font-semibold text-white transition hover:bg-emerald-700 sm:text-base">
        ...
        <Reveal>
          <div className="mt-8 bg-white p-6 dark:bg-gray-800">
            <h2 className="text-2xl font-semibold text-neutral-900 dark:text-white">Motivation Corner</h2>
            <div className="mt-4 grid gap-4 md:grid-cols-3">
              {quotes.map((quote, i) => (
                <Reveal key={quote} direction="scale" delay={i * 100}>
                  <blockquote
                    className="rounded-2xl bg-emerald-50 p-4 text-sm italic text-neutral-700 sm:text-base dark:bg-emerald-900/20 dark:text-neutral-300"
                  >
                    "{quote}"
                  </blockquote>
                </Reveal>
        ...
        <Reveal>
          <div className="mt-8 bg-emerald-900 p-6 text-white dark:bg-emerald-950">
            ...
            <div className="mt-5 bg-emerald-800 p-5 sm:p-6 dark:bg-emerald-900">
        ...
        <Reveal direction="scale">
          <div className="mt-8 bg-white p-6 dark:bg-gray-800">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h2 className="text-2xl font-semibold text-neutral-900 dark:text-white">Ready to Start Your Preparation?</h2>
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <button className="rounded-full px-5 py-2.5 text-sm font-semibold text-emerald-700 transition hover:bg-emerald-50 sm:text-base dark:text-emerald-400 dark:hover:bg-emerald-900/20">
                  Talk to Counsellor
                </button>
              </div>
            </div>
